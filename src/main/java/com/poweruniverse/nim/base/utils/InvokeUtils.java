	...
	public static Result invokeService(InvokeEnvelope invokeEnvelope){
		...
		try {
			...
			if(invokeEnvelope.isInnerInvoke()){
				...
				/*以下调用带参的、私有构造函数*/   
				Object serviceInstance = invokeEnvelope.getTargetWebservice().getInstance();
	            if(serviceInstance == null){
	            	serviceInstance = Class.forName(((LocalWebservice)invokeEnvelope.getTargetWebservice()).getServiceClass()).newInstance();
	            	invokeEnvelope.getTargetWebservice().setInstance(serviceInstance);
	            }
				ret = doMethodInvokeByParams(serviceInstance,targetMethod,invokeEnvelope);
			}else{
				//使用反射方法 取得构建方法 创建webservice实例
				Object servicePortInstance = invokeEnvelope.getTargetWebservice().getInstance();
	            if(servicePortInstance == null){
					...
					params[0] = new URL(InvokeUtils.class.getResource("."),remoteWebservice.getRemoteWebserviceWSDL() );
	            	...
	            	invokeEnvelope.getTargetWebservice().setInstance(servicePortInstance);
	            }
				...
				provider.getRequestContext().put(BindingProvider.USERNAME_PROPERTY, invokeEnvelope.getInvokeUser().getCodeIdentifier());
				provider.getRequestContext().put(BindingProvider.PASSWORD_PROPERTY, invokeEnvelope.getInvokeUser().getPwdIdentifier());
				//用客户端传递来的参数 调用webservice方法
				ret = doMethodInvokeByParams(servicePortInstance,targetMethod,invokeEnvelope);
			}
		} catch (Exception e) {
			e.printStackTrace();
			ret = new JSONMessageResult("访问服务器出错："+e.getMessage());
		}
		return ret;
	}
